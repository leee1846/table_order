...
| 역할 | 경로 |
|------|------|
| **광고 공통 모듈 (barrel)** | `apps/menu/src/feature/ad/index.ts` |
...
| 데이터 로드·영상 다운로드 | `apps/menu/src/feature/ad/useAdData.ts` |
| adType별 그룹화·순서 결정 | `apps/menu/src/feature/ad/groupAdFiles.ts` |
| 광고 파일 상태 스토어 | `apps/menu/src/stores/useAdStore.ts` |
| 전면 대기 광고 노출 상태 | `apps/menu/src/stores/useStandbyAdStore.ts` |
| 슬라이더 공통 컴포넌트 | `apps/menu/src/feature/ad/AdMediaSlider/index.tsx` |
...
| 주문완료 광고 노출 결정 유틸 | `apps/menu/src/feature/ad/orderCompleteAdVisibility.ts` |
...
interface IGetMenuAdFile {
  ...
  durationSec: number;
  fileSizeKb: number;
  sortOrder: number;     // 서버 메타데이터 (클라이언트 슬라이드 순서에는 미사용)
...
5. setAdFiles(validFiles)
   └─ adType별로 groupAdFiles() → API 배열 순서 유지
   └─ ORDER_COMP FULL/SIDE 공존 시 배열에서 먼저 등장한 슬롯만 store에 반영
...
```ts
// apps/menu/src/feature/ad/AdMediaSlider/index.tsx
...
```ts
// apps/menu/src/feature/ad/orderCompleteAdVisibility.ts

shouldShowOrderCompleteFullscreenAd(fullFiles): boolean
  // groupAdFiles에서 FULL 슬롯이 선택된 경우 (fullFiles.length > 0)
  → true이면 전면 광고만 표시
...
shouldShowOrderCompleteCountdown(fullFiles): boolean
...
FULL과 SIDE가 동시에 존재할 때는 `groupAdFiles`가 API 배열에서 먼저 등장한 슬롯만 store에 남긴다.
`orderCompleteAdVisibility`는 store에 남은 배열 길이만 확인한다.
...
- **슬라이드 순서:** 모든 adType 슬롯은 API 응답 배열 순서를 따른다. `sortOrder` 필드는 클라이언트 정렬에 사용하지 않는다.

- **FULL/SIDE 동시 표시 없음:** `groupAdFiles`가 API 배열에서 먼저 등장한 ORDER_COMP 슬롯만 store에 반영한다. 두 슬롯이 동시에 표시되는 경우는 없다.
